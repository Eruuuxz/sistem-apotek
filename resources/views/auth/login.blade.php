...
<head>
    ...
    <title>Login {{ isset($role) ? ucfirst($role) : '' }} - Sistem Apotek</title>
...
<body class="bg-white">
    ...
    @php
        // Menentukan tema berdasarkan peran (role)
        $is_kasir = isset($role) && $role === 'kasir';
    @endphp

    <div class="flex flex-wrap min-h-screen">
        
        <div class="hidden lg:flex w-1/2 items-center justify-center p-12 text-white
            ...
            <div class="w-full max-w-md text-center">
                ...
                <p class="{{ $is_kasir ? 'text-green-200' : 'text-blue-200' }}">
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus
                               class="block w-full px-4 py-3 border border-slate-300 rounded-lg text-slate-900 placeholder:text-slate-400 focus:outline-none 
                                      {{ $is_kasir ? 'focus:border-green-500 focus:ring-1 focus:ring-green-500' : 'focus:border-blue-500 focus:ring-1 focus:ring-blue-500' }}">
                    </div>
                    ...
                    <div class="flex items-center justify-between text-sm">
                        <label for="remember" class="flex items-center gap-2">
                            <input id="remember" type="checkbox" name="remember" 
                                   class="h-4 w-4 rounded border-slate-300 {{ $is_kasir ? 'text-green-600 focus:ring-green-500' : 'text-blue-600 focus:ring-blue-500' }}">
                            <span class="text-slate-600">Ingat saya</span>
                        </label>
                        <a href="{{ route('password.request') }}" class="font-medium {{ $is_kasir ? 'text-green-600' : 'text-blue-600' }} hover:underline">
                            Lupa password?
                        </a>
                    </div>
                    ...
                    <button type="submit"
                            class="w-full text-white py-3 px-4 rounded-lg font-semibold transition-colors duration-200 
                                   ...
                                   {{ $is_kasir ? 'bg-green-600 hover:bg-green-700 focus:ring-green-500 shadow-lg shadow-green-500/20 hover:shadow-xl hover:shadow-green-500/40' : 'bg-blue-600 hover:bg-blue-700 focus:ring-blue-500 shadow-lg shadow-blue-500/20 hover:shadow-xl hover:shadow-blue-500/40' }}">
                        Login
                    </button>

                     @error('email')
                        <p class="text-red-500 text-sm text-center pt-2">{{ $message }}</p>
                    @enderror
                </form>
