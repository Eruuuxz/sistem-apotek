    ...
    <form action="{{ route('pelanggan.update', $pelanggan) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <textarea name="alamat" id="alamat" rows="3"
                class="w-full border rounded px-3 py-2 @error('alamat') border-red-500 @enderror">{{ old('alamat', $pelanggan->alamat) }}</textarea>
            @error('alamat')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="no_ktp" id="no_ktp" value="{{ old('no_ktp', $pelanggan->no_ktp) }}"
        ...
        <div class="mb-4">
            ...
            @if($pelanggan->file_ktp)
                <div class="mb-2">
                    <p class="text-sm text-gray-600">File KTP saat ini:</p>
                    <img src="{{ Storage::url($pelanggan->file_ktp) }}" alt="KTP" class="w-32 h-20 object-cover border rounded">
                </div>
            @else
                <p class="text-sm text-gray-600 mb-2">Tidak ada file KTP terupload.</p>
            @endif
        ...
        <div class="mb-6">
            ...
            <input type="number" name="point" id="point" value="{{ old('point', $pelanggan->point) }}" min="0"
