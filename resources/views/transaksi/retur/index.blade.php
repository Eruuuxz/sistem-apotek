...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <a href="{{ route('retur.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition">+ Tambah Retur</a>
    </div>
    ...
    <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 bg-white p-4 rounded shadow">
        ...
        <div>
            ...
            <input type="date" name="from" id="from_date" value="{{ request('from') }}" class="w-full border rounded px-2 py-1">
        </div>
        <div>
            <label for="to_date" class="block text-sm font-medium text-gray-700 mb-1">Sampai Tanggal</label>
            <input type="date" name="to" id="to_date" value="{{ request('to') }}" class="w-full border rounded px-2 py-1">
        </div>
        <div class="flex gap-2 items-end">
            <button type="submit" class="bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700 transition">Filter</button>
            <a href="{{ route('retur.index') }}" class="bg-gray-400 text-white px-3 py-1 rounded hover:bg-gray-500 transition">Reset</a>
        </div>
    </form>
