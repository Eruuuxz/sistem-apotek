...
@section('content')
    ...
    <div class="bg-white p-8 shadow-xl rounded-xl max-w-3xl mx-auto mt-6">
        <div class="mb-8">
            ...
            <p class="text-sm text-gray-500">Isi detail di bawah ini untuk menambahkan obat baru.</p>
        </div>
        ...
        <form action="{{ route('obat.store') }}" method="POST" class="space-y-6">
            ...
            <div>
             ...
             {{-- Stok & Harga (Bagian Kemasan dihapus) --}}
             <div class="border-t pt-6 space-y-6">
                 <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                     <div>
                        <label class="block mb-2 text-sm font-semibold text-gray-700">Stok Awal</label>
                        <input type="number" name="stok" placeholder="0" value="{{ old('stok') }}" required class="w-full border rounded-lg px-4 py-2">
                    </div>
                     <div>
                 ...
                 <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        {{-- MODIFIKASI: step="0.01" agar browser terima desimal --}}
                        <input type="number" step="0.01" id="harga_dasar" name="harga_dasar" placeholder="0" value="{{ old('harga_dasar') }}" required class="w-full border rounded-lg px-4 py-2">
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-semibold text-gray-700">Margin Untung (%)</label>
                        {{-- MODIFIKASI: step="0.01" agar browser terima desimal --}}
                        <input type="number" step="0.01" id="persen_untung" name="persen_untung" placeholder="0" value="{{ old('persen_untung') }}" class="w-full border rounded-lg px-4 py-2">
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-semibold text-gray-700">Harga Jual (Rp)</label>
                        {{-- MODIFIKASI: step="0.01" agar browser terima desimal --}}
                        <input type="number" step="0.01" id="harga_jual" name="harga_jual" value="{{ old('harga_jual') }}" required class="w-full border rounded-lg px-4 py-2 bg-gray-50 font-bold">
                    </div>
                </div>
            </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            function updateHargaJual() {
                ...
                if (dasar > 0) {
                    // Pembulatan ke integer agar rapi untuk Rupiah, tapi input tetap support desimal
                    let hitung = Math.round(dasar + (dasar * persen / 100));
                    hargaJual.value = hitung;
                } else {
                     hargaJual.value = '';
                }
            }
